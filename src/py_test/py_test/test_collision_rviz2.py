...
def build_models_from_urdf(urdf_string: str, mesh_dir: Path, use_freeflyer=False):
    # 选择 joint model 类型
...
class Test_Collision_Rviz2_Node(Node):
    def __init__(self):
        super().__init__('test_collision_rviz2')
        
        self.urdf_sub_ = self.create_subscription( String, '/robot_description', self.urdf_callback, 10)
        
        self.get_logger().info("Test_Collision_Rviz2_Node 节点已启动。")
    def urdf_callback(self, msg):
        self.get_logger().info("收到 URDF 描述。")
        urdf_string = msg.data
        # 解析 URDF，初始化 Pinocchio 模型等
        # ...
        self.get_logger().info("URDF 解析完成。")
        # 取消订阅，避免重复处理
        # self.destroy_subscription(self.urdf_sub_)

def main(args=None):
    rclpy.init(args=args)
    test_collision_rviz2_node = Test_Collision_Rviz2_Node()
    rclpy.spin(test_collision_rviz2_node)
    ...
    # print("测试完成。")
    # input("按回车开始...")
    test_collision_rviz2_node.destroy_node()
    rclpy.shutdown()
